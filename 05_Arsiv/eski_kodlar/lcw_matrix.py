...
"""
LCW-Matrix v1.0
LC Waikiki Pilot İl — Mesafe ve Süre Matrisi Üreticisi

Girdi : LCW_Pilot_Lokasyonlar.xlsx
Çıktı : LCW_Mesafe_Sure_Matrisi.xlsx  (Mesafe km + Süre dk)
...
INPUT_FILE  = "LCW_Pilot_Lokasyonlar.xlsx"
OUTPUT_FILE = "LCW_Mesafe_Sure_Matrisi.xlsx"
LOG_DIR     = Path("logs")

# OSRM — kamu sunucusu. Kendi kurulumunuz varsa BASE_URL'i değiştirin.
OSRM_BASE_URL = "http://router.project-osrm.org/table/v1/driving"

# Kaynak (source) grubu başına lokasyon sayısı.
# OSRM genel sunucusunun HTTP 400 (aşırı yük) hatası vermemesi için
# blok boyutu 50'den 10'a düşürüldü.
BATCH_SIZE      = 10
REQUEST_TIMEOUT = 120   # saniye
MAX_RETRIES     = 3
RETRY_WAIT      = 15    # saniye
...
def setup_logging() -> logging.Logger:
    ...
    fh  = logging.FileHandler(LOG_DIR / f"matrix_{ts}.log", encoding="utf-8")
    sh  = logging.StreamHandler()
    fmt = logging.Formatter("%(asctime)s [%(levelname)s] %(message)s")
    fh.setFormatter(fmt); sh.setFormatter(fmt)
    logger = logging.getLogger("lcw-matrix")
...
def load_locations(path: str, logger: logging.Logger) -> pd.DataFrame:
...
def build_labels(df: pd.DataFrame) -> list[str]:
    """
    Matris satır/sütun başlığı: "{sıra}. {İl} — {İlçe}"
    İlçe boşsa sadece il adı kullanılır.
    """
    labels = []
...
class OsrmClient:
    ...
    # ── Koordinat dizesini oluştur (OSRM: lon,lat sırası) ──
    ...
    # ── Tam matris: kaynak gruplara böl, parçaları birleştir ──

    def compute_matrix(
        self, df: pd.DataFrame
    ) -> tuple[list[list[Optional[float]]], list[list[Optional[float]]]]:
        """
        N×N mesafe (km) ve süre (dk) matrislerini döndürür.
        Eksik değerler None olarak işaretlenir.
        """
        n           = len(df)
        ...
        for batch_idx, source_indices in enumerate(source_batches, 1):
            ...
            if data is None:
                self.logger.error(
                    f"  Grup {batch_idx} başarısız — "
                    f"bu kaynaklar için değerler None kalacak."
                )
                continue
            ...
            # Gruplar arası kısa bekleme (kamu sunucusu kibarlığı)
...
class MatrixExporter:
    ...
    def _write_matrix_sheet(
        ...
        # Sütun başlıkları (satır 1, sütun 2..N+1)
        ...
        # Renk skalası (veri hücreleri): yeşil → sarı → kırmızı
        data_range = (
            f"B2:{get_column_letter(n + 1)}{n + 1}"
        )
...
def main() -> None:
    ...
    logger.info("=" * 62)
    logger.info("LCW-Matrix v1.0 başlatıldı")
    logger.info("=" * 62)

    # ── ADIM 1: Lokasyonları oku ─────────────────────────────────────────────
    ...
    if none_count > 0:
        logger.warning(
            "Hesaplanamayan çiftler Excel'de boş hücre olarak görünecek. "
            "OSRM sunucusu bu rotaları bulamadı."
        )

    # ── ADIM 4: Excel dışa aktarım ───────────────────────────────────────────
